...
export const MobileChatDrawer: React.FC<MobileChatDrawerProps> = ({
  ...
  return (
    <div 
      ref={drawerRef}
      className={`fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-200 transition-all duration-300 ease-out shadow-[0_-4px_20px_rgba(0,0,0,0.15)] ${isExpanded ? "h-[55vh]" : "h-auto"}`}
      style={{
        zIndex: 60,
    ...
    >
      ...
      {/* Input Area */}
      <div className={`px-2 pb-1.5 ${isExpanded ? "pt-2 border-t border-gray-200" : "pt-0.5"}`}>
        {isListening && (
